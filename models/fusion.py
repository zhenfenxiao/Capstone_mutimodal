...
import math
from cv2 import _OUTPUT_ARRAY_DEPTH_MASK_16F, initUndistortRectifyMap, log
...
class Fusion(nn.Module):
    ...
    def __init__(self, ehr_dim,model_depth=50, cardinality=32, num_channels=3, num_classes=600, init_method=None,
        ...
        self.elastic=nn.Linear(in_features=ehr_dim,out_features=1)
        if init_method is not None:
            self._initialize_elastic(init_method)
            self._initialize_penet(init_method,focal_pi=0.01)
            
    def _initialize_elastic(self,init_method,gain=0.2):
        """Initialize  weights in the elasticnet."""
        if init_method == 'normal':
            nn.init.normal_(self.elastic.weight,mean=0,std=gain)
        elif init_method == 'xavier':
            nn.init.xavier_normal_(self.elastic.weight,gain=gain)
        elif init_method == 'kaiming':
            nn.init.kaiming_normal_(self.elastic.weight)
        else:
            raise NotImplementedError('Invalid initialization method: {}'.format(self.init_method))
    def _initialize_penet(self, init_method, gain=0.2, focal_pi=None):
        """Initialize  weights in the penet."""
    ...
    def _calculate_l1_loss_elstic(self,alpha):
        loss=0
        for param in  self.elastic.parameters():
            loss+=torch.sum(torch.abs(param))
        return alpha*loss
    def forward(self, x, ehr):
        ...
        # Classifier
        x = self.classifier(x)#penet

        ehr_prob=self.elastic(ehr)
        
        ehr_prob=torch.sigmoid(ehr_prob).squeeze(-1)
        image_prob=torch.sigmoid(x)
        output=(ehr_prob+image_prob)/2
        output=torch.logit(output)
        return output
