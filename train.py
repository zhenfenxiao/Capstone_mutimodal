...
def train(args):
    ...
    if args.ckpt_path and not args.use_pretrained:
        ...
        ##this is our path
        model_fn = models.__dict__[args.model]##our choice is PENETClassifier
    ...
    # Get model for EHR data:ealstic
    ehr_model=elastic.ElasticNet(input_dim=2912)
    ...
    optimizer = torch.optim.Adam([{'params':model.parameters()},{'params':ehr_model.parameters()}], lr=args.learning_rate,
    ...
    while not logger.is_finished_training():
        ...
        for inputs, target_dict ,ehr_data in train_loader:
            logger.start_iter()
            with torch.set_grad_enabled(True):
                inputs.to(args.device)
                cls_logits = model.forward(inputs)
                ehr_prob = ehr_model.forward(ehr_data)
                ehr_prob=ehr_prob.squeeze(-1)
                print('ehr shape: ',ehr_prob.shape)
                print('penet shape: ',cls_logits.shape)
                cls_prob=1/(1+torch.exp(-cls_logits))

                fusion_prob=(cls_prob+ehr_prob)/2

                fusion_logits=torch.logit(fusion_prob)

                cls_targets = target_dict['is_abnormal']
            
                ehr_parameters=[]
                for para in ehr_model.parameters():
                    ehr_parameters.append(para.view(-1))
                l1=ehr_model.cal_l1_loss(torch.cat(ehr_parameters))

                # no need to calculate l2 loss,because we use weight_decay in the optimizer
                # l2=ehr_model.cal_l2_loss(torch.cat(ehr_parameters))
                

                
                #cls_loss = cls_loss_fn(cls_logits, cls_targets.to(args.device))
                cls_loss = cls_loss_fn(fusion_logits, cls_targets.to(args.device))
                #loss = cls_loss.mean()
                loss=cls_loss.mean()+l1
